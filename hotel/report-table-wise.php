                ...
                <div class="card shadow mb-4">
                    <?php 
                        ...
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold float-left text-primary">Table Wise Report</h6>
...
<!--                                 <div class="form-group col-md-3">
                        ...
                        <div class="table-responsive table-wrapper report-table" id="tab">
                            <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                <?php
                                    $table_main_sql = "SELECT * FROM `new_table_master` ORDER BY `table_no`";
                                    
                                    $table_main_result = mysqli_query($conn, $table_main_sql);
                                    ...
                                    while($table_main_data = mysqli_fetch_assoc($table_main_result)) {
                                        // echo "hi";

                                        $count = $count + 1;
                                        $table_item_total = 0;
                                        $table_discount_total = 0;
                                        $table_grand_total = 0;

                                        if (isset($_POST['date_submit'])) {
                                            $table_no = $table_main_data['table_id'];
                                            
                                            $selected_date = date('Y-m-d', strtotime($_POST['end_date']));

                                            $select_sql = "SELECT * FROM `tbl_bills` WHERE `bill_status` = 1 && `table_no` = '$table_no' &&`bill_date` = '$selected_date' ORDER BY `table_no`";
                                            
                                        } else {
                                            $table_no = $table_main_data['table_id'];
                                            $current_date = date('Y-m-d');

                                            $select_sql = "SELECT * FROM `tbl_bills` WHERE `bill_status` = 1 && `table_no` = '$table_no' &&`bill_date` = '$current_date' ORDER BY `table_no`";
                                        }
                                        ...
                                        while ($data = mysqli_fetch_assoc($result)) {
                                            // echo "gdsad";
                                            $table_item_total += $data['sub_total'];
                                            $table_discount_total += $data['discount'];
                                            $table_grand_total += $data['grand_total'];
                                        }
                                        $total_item_total += $table_item_total;
                                        $total_discount += $table_discount_total;
                                        $total_grand_total += $table_grand_total;

                                        echo '<tr>';
                                            echo "<td nowrap>".$count."</td>";
                                            echo "<td nowrap>".$table_main_data['table_no']."</td>";
                                            echo "<td nowrap>".$table_item_total."</td>";
                                            echo "<td nowrap>".$table_discount_total."</td>";
                                            echo "<td nowrap>".$table_grand_total."</td>";
                                        echo '</tr>';
                                    }
                                    echo '</tbody>';
                                    echo '<tfoot>';
