<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET') {
		...
		if (isset($_GET['latitude']) && isset($_GET['longitude']) && isset($_GET['distance'])) {
			...
			$select_sql = "SELECT * , (3956 * 2 * ASIN(SQRT( POWER(SIN(( '$latitude' - `latitude`) *  pi()/180 / 2), 2) + COS( '$latitude' * pi()/180) * COS(`latitude` * pi()/180) * POWER(SIN(( '$longitude' - `longitude`) * pi()/180 / 2), 2) ))) as distance from `tbl_store` having distance <= '$distance' order by distance";
	        ...
	        $double = array(
							'msg' => 'Unable to Get Nearby Stores',
							'statusCode' => 503
							);
